...
export const SourcingPage = () => {
  ...
  return <>
    ...
    <div className="flex gap-3 overflow-x-auto pb-3">{stages.map((stage) => <section key={stage} className="min-w-80 rounded-2xl border border-slate-200 bg-slate-50 p-3"><div className="mb-3 flex justify-between"><h2 className="font-bold">{stage}</h2><Badge tone="slate">{sourcing.filter((item) => item.stage === stage).length}</Badge></div>{sourcing.filter((item) => item.stage === stage).map((item) => <article key={item.id} className="mb-3 rounded-2xl bg-white p-3 shadow-sm"><h3 className="font-bold">{item.item}</h3><p dir="auto" className="text-sm text-slate-500">{item.customer} · {item.cn}</p><p className="mt-2 text-xs text-slate-500">{item.qty} · {item.candidates.join(" / ")}</p><div className="mt-3 flex gap-2"><Badge tone="amber">¥{item.priceRmb}</Badge><Badge tone="green">{rmbToUsd(item.priceRmb, exchangeRate)}</Badge></div><div className="mt-3"><Button tone="secondary" onClick={() => advanceSourcing(item.id)}>Advance →</Button></div></article>)}</section>)}</div>
  </>
}
