...
export const AppShell = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="min-h-screen bg-[#f5f3ee] text-[#211f1b]">
      <aside className="fixed inset-y-0 left-0 z-30 flex w-[54px] flex-col items-center bg-[#1f1e1b] text-[#8d877c] shadow-[1px_0_0_rgba(0,0,0,0.2)]">
        <div className="mt-4 grid size-8 place-items-center rounded-xl bg-white text-[15px] font-black text-[#1f1e1b] shadow-sm">H</div>
          ...
          {railItems.map((item) => {
            ...
            return (
              <Link key={`${item.href}-${item.label}`} href={item.href} className={`relative flex w-full flex-col items-center gap-1 py-2.5 text-[9px] transition ${active ? "text-white" : "hover:text-white"}`}>
                <span className={`grid size-8 place-items-center rounded-xl ${active ? "bg-[#3c382f] shadow-inner" : ""}`}>{item.icon}</span>
        ...
        <div className="mb-3 flex w-full flex-col items-center gap-2 text-[9px]">
          ...
          <div className="mt-1 grid size-8 place-items-center rounded-full bg-[#3c382f] text-[10px] font-bold text-white">MW</div>
      ...
      <aside className="fixed inset-y-0 left-[54px] z-20 w-[142px] border-r border-[#e4e0d6] bg-[#f5f3ee] px-3 py-4">
        <div className="mb-5 pl-1">
          <h2 className="text-[15px] font-extrabold tracking-tight">Importing</h2>
          <p className="mt-1 text-[9px] font-bold uppercase tracking-[0.16em] text-[#78736a]">Trade operations</p>
        </div>
        <nav className="space-y-1">
          {moduleItems.map((item) => {
            const active = pathname === item.href || (item.href === "/dashboard" && (pathname === "/" || pathname === "/overview"))
            return (
              <Link key={item.href} href={item.href} className={`flex h-9 items-center justify-between rounded-lg px-2 text-[11px] font-semibold transition ${active ? "bg-white text-blue-700 shadow-sm" : "text-[#3c382f] hover:bg-white/70"}`}>
        ...
        <div className="absolute bottom-4 left-3 right-3 rounded-lg bg-[#ebece6] px-3 py-2 text-[10px] text-[#6b7066]">
          <p className="flex items-center gap-1 font-medium text-[#2f8f63]"><span className="size-2 rounded-full bg-emerald-500" /> Facebook sync active</p>
